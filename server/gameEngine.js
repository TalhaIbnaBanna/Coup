...
class CoupGame {
  constructor(settings = {}) {
    ...
    this.timers = {};
    this.winner = null;
    this.lastEvent = null; // Most recent significant event for banner display
  }
  ...
  addEvent(message, isBanner = false) {
    this.eventLog.push({
      message,
      timestamp: Date.now()
    });
    if (isBanner) {
      this.lastEvent = { message, timestamp: Date.now() };
    }
  }
  ...
  getAvailableActions(playerId) {
    ...
    if (this.phase === 'block_window') {
      ...
      if (!this.turnData.respondedPlayers?.includes(playerId) && this.turnData.actorId !== playerId) {
        if (this.turnData.claimedCharacter) {
  ...
  processAction(playerId, action, targetId = null) {
    ...
    switch (action) {
      case 'income':
        player.coins += 1;
        this.addEvent(`${player.name} takes Income (+1 coin).`, true);
        this.nextTurn();
        return { success: true, resolved: true };

      case 'foreign_aid':
        this.addEvent(`${player.name} attempts Foreign Aid (+2 coins).`, true);
      ...
      case 'coup':
        ...
        player.coins -= 7;
        this.addEvent(`${player.name} launches a Coup against ${coupTarget.name}!`, true);
        this.turnData.choosingPlayerId = targetId;
        this.phase = 'choose_card';
        return { success: true, phase: 'choose_card' };

      case 'tax':
        this.addEvent(`${player.name} claims Duke and takes Tax (+3 coins).`, true);
      ...
      case 'assassinate':
        ...
        player.coins -= 3;
        this.addEvent(`${player.name} claims Assassin and attempts to assassinate ${assTarget.name}!`, true);
      ...
      case 'steal':
        ...
        this.addEvent(`${player.name} claims Captain and attempts to steal from ${stealTarget.name}.`, true);
      ...
      case 'exchange':
        this.addEvent(`${player.name} claims Ambassador and wants to exchange cards.`, true);
  ...
  handleChallenge(playerId) {
    ...
    if (hasCharacter) {
      ...
      this.addEvent(`${actor.name} reveals ${CHARACTER_INFO[claimedChar].name}! Challenge failed!`, true);
      ...
      this.addEvent(`${actor.name} was bluffing! Challenge successful!`, true);
  ...
  handleBlock(playerId, character) {
    ...
    this.addEvent(`${blocker.name} claims ${CHARACTER_INFO[character].name} and blocks ${actor.name}!`, true);
  ...
  handleBlockSuccess() {
    ...
    this.addEvent('The action was blocked.', true);
    this.nextTurn();
  }
  ...
  checkWinner() {
    ...
    if (alive.length <= 1) {
      ...
      if (this.winner) {
        this.addEvent(`🏆 ${this.winner.name} wins the game!`);
      } else {
        this.addEvent('Game Over — No players left alive.');
      }
      return true;
    }
    return false;
  ...
  nextTurn() {
    ...
    do {
      ...
      safety++;
      if (safety > this.players.length * 2) break;
    ...
    if (currentPlayer) {
      this.addEvent(`${currentPlayer.name}'s turn.`);
    }
  }
